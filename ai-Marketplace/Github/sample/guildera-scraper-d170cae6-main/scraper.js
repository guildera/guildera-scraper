    ...
    for (let i = 0; i < articleCount; i++) {
      ...
      try {
        ...
        try {
          ...
          }, await tweet.elementHandle());
          await new Promise(r => setTimeout(r, 1500));

          try {
            await page.evaluate(el => {
              const imgs = el.querySelectorAll('img[loading="lazy"]');
              imgs.forEach(img => {
                img.loading = 'eager';
          ...
          const imgSelectors = [
            ...
            'img[src*="pbs.twimg.com/amplify_video"]',
            '[data-testid="tweetPhoto"] img',
            '[data-testid="gamePlayer"] img',
            'div[role="group"] img[src*="pbs.twimg"]',
            'a[href*="/status/"] img[src*="pbs.twimg"]',
          ];
          const imgEls = tweet.locator(imgSelectors.join(', '));
          const imgCount = await imgEls.count({ timeout: 3000 });
          for (let m = 0; m < imgCount; m++) {
            try {
              const el = imgEls.nth(m);
              let src = await el.getAttribute('src', { timeout: 1500 });
              if (!src) {
                src = await el.getAttribute('data-src', { timeout: 500 }).catch(() => null);
              }
              if (!src) {
                const srcset = await el.getAttribute('srcset', { timeout: 500 }).catch(() => null);
                if (srcset) {
                  const parts = srcset.split(',').map(s => s.trim().split(' ')[0]);
                  src = parts[parts.length - 1] || parts[0] || null;
                }
              }
              if (src && !src.includes('profile_images') && !src.includes('emoji') && !src.includes('analytics') && src.includes('pbs.twimg.com')) {
                var clean = src.includes('?') ? src : src + '?format=jpg&name=large';
          ...
          const videoSelectors = [
            ...
            'video[src]',
            'video source[src*="pbs.twimg"]',
          ];
          for (const vsel of videoSelectors) {
            try {
              const vc = await tweet.locator(vsel).count({ timeout: 1000 });
              if (vc > 0) {
                const videoEl = tweet.locator(vsel + ' video').first();
                const videoSrc = await videoEl.getAttribute('src', { timeout: 1000 });
          ...
          const cardSelectors = [
            ...
            '[data-testid="externalLinkCard"] img',
            '[data-testid="cardLayoutLarge"] img',
            'div[role="link"] img[src*="pbs.twimg"]',
          ];
          const cardImg = tweet.locator(cardSelectors.join(', '));
          try {
            const cardCount = await cardImg.count({ timeout: 1000 });
            for (let c = 0; c < cardCount; c++) {
              try {
                const src = await cardImg.nth(c).getAttribute('src', { timeout: 1000 });
                if (src && !src.includes('profile_images') && !src.includes('emoji') && src.includes('pbs.twimg.com')) {
                  var clean = src.includes('?') ? src : src + '?format=jpg&name=large';
                  if (!seenUrls.has(clean)) {
                    seenUrls.add(clean);
                    mediaUrls.push(clean);
        ...
        try {
          ...
          if (hasGroup) {
            ...
            if (nums.length >= 5) {
              ...
              viewCount = parseEngagementNum(nums[3]);
        ...
        if (viewCount === 0) {
          try {
            const viewEl = tweet.locator('[data-testid="viewCount"], [data-testid="Views"]').first();
            const cnt = await viewEl.count({ timeout: 500 });
            if (cnt) {
              const label = await viewEl.getAttribute('aria-label', { timeout: 500 });
              const textContent = await viewEl.textContent({ timeout: 500 });
              viewCount = parseEngagementNum(label || textContent || '');
            }
          } catch(e2) {}
        }
        ...
        let isReply = false;
        try {
          const replyLine = tweet.locator('text=Replying to').first();
          isReply = await replyLine.count({ timeout: 1000 }) > 0;
        } catch(e2) {}
